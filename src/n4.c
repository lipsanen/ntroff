...
extern int nroff_env_ad, nroff_env_admod;
extern int print;
...
setn()
{
	...
	if((i=getsn()) == 0)return 0;
	if((i & 0177) == '.')switch(i){
		case '.s': i = nroff_env.pts & 077;	break;
		case '.v': i = nroff_env.lss;		break;
		case '.f': i = nroff_env.font + 1;	break;
		case '.p': i = pl;		break;
		case '.t':  i = findt1();	break;
		case '.o': i = po;		break;
		case '.l': i = nroff_env.ll;		break;
		case '.i': i = nroff_env.in;		break;
		case '.$': i = *frame;		break;
		case '.A': i = ascii;		break;
		case '.c': i = v.cd;		break;
		case '.n': i = nroff_env.lastl;		break;
		case '.a': i = ralss;		break;
		case '.h': i = dip->hnl;	break;
		case '.d':
			if(dip->op)i = dip->dnl; else i = v.nl;
			break;
		case '.u': i = nroff_env.fi;		break;
		case '.j': i = nroff_env.ad + 2*nroff_env.admod;	break;
		case '.w': i = cwidth;		break;
		case '.x': i = ulfont + 1;	break;
		case '.y': i = nroff_env.font1;		break;
		case '.T': i = dotT;		break; /*-Tterm used in nroff*/
		case '.V': i = VERT;		break;
		case '.H': i = HOR;		break;
		case '.k': i = nroff_env.ne;		break;
		case '.P': i = print;		break;
		case '.L': i = nroff_env.ls;		break;
		case '.z': i = dip->curd;
			cbuf[0] = i & BMASK;
			cbuf[1] = (i >> BYTE) & BMASK;
			cbuf[2] = 0;
