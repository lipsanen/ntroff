...
extern int nroff_env_tabc, nroff_env_dotc;
...
setvline(){
	...
	if(((c = getch()) & CMASK) == delim){
		c = 0337 | nroff_env.chbits;	/*default box rule*/
	}else getch();
	c |= ZBIT;
	neg = 0;
	if(i < 0){
...
casefc(){
	...
	if(skip() ||
	   ...
	   ((i &= CMASK) == '\n'))return 0;
	fc = i;
	if(skip() || (ch & MOT) || ((ch &= CMASK) == fc))return 0;
	padc = ch;
}
setfield(x)
int x;
{
	...
	if(x == tabch) rchar = nroff_env.tabc | nroff_env.chbits;
	else if(x ==  ldrch) rchar = nroff_env.dotc | nroff_env.chbits;
	temp = npad = ws = 0;
	savfc = fc; savtc = tabch; savlc = ldrch;
	tabch = ldrch = fc = IMP;
	for(j=0;;j++){
		if((nroff_env.tabtab[j] & TMASK)== 0){
			if(x==savfc)prstr("Zero field width.\n");
			j = 0;
			goto rtn;
		}
		if((length = ((nroff_env.tabtab[j] & TMASK) - v.hp)) > 0 )break;
	}
	type = nroff_env.tabtab[j] & (~TMASK);
	fp = fbuf;
	pp = padptr;
	if(x == savfc){while(1){
