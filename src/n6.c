...
mchbits(){
	...
	nroff_env.chbits = (((nroff_env.pts)<<2) | nroff_env.font) << (BYTE + 1);
	nroff_env.sps = width(' ' | nroff_env.chbits);
}
setps(){
	register i,j;
...
int a;
{
	...
	if(i == 'S')return 0;
	if((j = find(i,fontlab))  == -1)return 0;
s0:
	nroff_env.font1 = nroff_env.font;
	nroff_env.font = j;
	mchbits();
}
setwd(){
	...
	if((delim = getch() & CMASK) & MOT)return 0;
	savhp = v.hp;
	savlevel = level;
	v.hp = level = 0;
...
casefp(){
	...
	skip();
	if(((i = (getch() & CMASK) - '0' -1) < 0) || (i >3))return 0;
...
casevs(){
	...
	dfactd = 72;
	res = VERT;
	i = inumb(&nroff_env.lss);
	if(nonumb)i = nroff_env.lss1;
	if(i < VERT)i = VERT;
	nroff_env.lss1 = nroff_env.lss;
	nroff_env.lss = i;
}
xlss(){
	register i, j;
