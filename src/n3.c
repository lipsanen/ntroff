...
casern(){
	...
	lgf++;
	skip();
	if(((i=getrq())==0) || ((oldmn=findmn(i)) < 0))return 0;
	skip();
...
{
	if(i >= 0){
		if(contab[i].rq & MMASK)ffree(contab[i].f);
...
int newip;
{
	...
	*p++ = ch0;
	*p++ = ch;
	cp = nchar = rchar = nroff_env.pendt = ap = ch0 = ch = 0;
	frame = nxf;
...
casedi(){
	...
	dip->curd = i;
	clrmn(oldmn);
	for(j=1; j<=10; j++)((int*)dip)[j] = 0;	/*not op and curd*/
rtn:
	app = 0;
	diflg = 0;
}
...
#endif
	hseg(pchar,begin);
	if(w2 || w3)horiz(j=quant((nroff_env.lt - w2)/2-w1,HOR));
	hseg(pchar,0);
	if(w3){
		horiz(nroff_env.lt-w1-w2-w3-j);
		hseg(pchar,0);
	}
	newline(0);
